...
enum class behavior { query, infer, plain, blocking };
                  ...
                  int batch)
{
    ...
    auto key = concat(dst_dims, src_dims, wgt_dims, false, false, (int)dnnl_dtype_16);
    ...
    if (i_compute == cached.end()) {
        ...
        if (alpha != 1.0f) { attr.set_scales_mask(DNNL_ARG_DST, /* mask */ 0); }
        if (beta != 0.0f) {
            dnnl::post_ops po;
            po.append_sum(beta);
