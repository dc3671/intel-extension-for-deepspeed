...
void launch_fused_ln(T* output,
                     ...
                     sycl::queue stream)
{
    // 8 for fp16, 4 for float
    constexpr int T_per_load = ln::granularity / sizeof(T);

    constexpr int maxThreads = 256;

    // For Flaoat, unRoll 4, for fp16, unRoll 2
...
template void
launch_fused_ln(fp16*, const fp16*, const fp16*, const fp16*, float, int, int, sycl::queue);
template void
...
template void launch_fused_residual_ln(fp16*,
                                       const fp16*,
                                       const fp16*,
                                       const fp16*,
                                       const fp16*,
...
template void launch_fused_residual_ln(float*,
                                       ...
                                       int,
                                       sycl::queue);
template <int unRoll, int threadsPerGroup, int maxThreads>
class fused_ln<bf16, unRoll, threadsPerGroup, maxThreads>;
template <int unRoll, int threadsPerGroup, int maxThreads>
class fused_ln<fp16, unRoll, threadsPerGroup, maxThreads>;
